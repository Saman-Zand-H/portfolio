...
def image_validators(file):
    ...
    if ext not in valid_exts:
        raise ValidationError("invalid image extension", "invalid_image")
...
class Article(models.Model):
    ...
    article = MDTextField()
    tags = models.ManyToManyField(Tag, blank=True, related_name="tag_articles")
    ...
    def parse_contents(self):
        ...
        def dfs(content, ctx=results):
            ...
            for heading in content.subheadings.all():
                ctx[f"{content.heading_text},{content.heading_id}"][
                    f"{heading.heading_text},{heading.heading_id}"
                ] = {}
...
class Heading(models.Model):
    article = models.ForeignKey(
        Article, on_delete=models.CASCADE, related_name="headings"
    )
